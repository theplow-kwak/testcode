...
static int nvme_copy_io_uring(struct io_uring *ring, struct nvme_copy_args *args, unsigned long user_data)
{
    ...
    cmd->data_len = args->nr * sizeof(struct nvme_copy_range); // format별로 조정 필요
...
static int copy_cmd(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	int inflight = 0, submitted = 0, completed = 0;
	int ret = 0;
	...
	// chunk 준비
	int n_chunks = (nr + chunk_size - 1) / chunk_size;
	for (int c = 0; c < n_chunks; ++c) {
		int off = c * chunk_size;
		int this_chunk = (off + chunk_size > nr) ? (nr - off) : chunk_size;
		...
		if (cfg.format == 0)
			nvme_init_copy_range((struct nvme_copy_range *)chunk_copy, &nlbs[off], &slbas[off], &eilbrts.short_pi[off], &elbatms[off], &elbats[off], this_chunk);
		else if (cfg.format == 1)
			nvme_init_copy_range_f1((struct nvme_copy_range_f1 *)chunk_copy, &nlbs[off], &slbas[off], &eilbrts.long_pi[off], &elbatms[off], &elbats[off], this_chunk);
		else if (cfg.format == 2)
			nvme_init_copy_range_f2((struct nvme_copy_range_f2 *)chunk_copy, &snsids[off], &nlbs[off], &slbas[off], &sopts[off], &eilbrts.short_pi[off], &elbatms[off], &elbats[off], this_chunk);
		else if (cfg.format == 3)
			nvme_init_copy_range_f3((struct nvme_copy_range_f3 *)chunk_copy, &snsids[off], &nlbs[off], &slbas[off], &sopts[off], &eilbrts.long_pi[off], &elbatms[off], &elbats[off], this_chunk);
		else {
			nvme_show_error("invalid format");
			ret = -EINVAL;
			free(chunk_copy);
		...
		chunk_ctx[c].chunk_copy = chunk_copy;
		chunk_ctx[c].chunk_idx = c;
		chunk_ctx[c].this_chunk = this_chunk;
		chunk_ctx[c].sdlba = cfg.sdlba + off;
	}

	// submit & complete loop (io_uring 기반 nvme_copy_io_uring 사용)
	while (completed < n_chunks && ret == 0) {
		// submit
		while (inflight < cfg.qdepth && submitted < n_chunks) {
			int idx = submitted;
			struct nvme_copy_args args = {
				.args_size = sizeof(args),
				.fd = dev_fd(dev),
				.nsid = cfg.namespace_id,
				.copy = chunk_ctx[idx].chunk_copy,
				.sdlba = chunk_ctx[idx].sdlba,
				.nr = chunk_ctx[idx].this_chunk,
				.prinfor = cfg.prinfor,
				.prinfow = cfg.prinfow,
				.dtype = cfg.dtype,
				.dspec = cfg.dspec,
			...
			if (nvme_copy_io_uring(&uring, &args, idx) < 0) {
		...
		int cqe_res = cqe->res;
		// CQE에서 cmd 포인터 해제 필요
		struct nvme_uring_cmd *cmd = NULL;
		io_uring_cqe_get_data64(cqe, (unsigned long *)&cmd); // liburing 2.3+에서 지원
		free(cmd);
		io_uring_cqe_seen(&uring, cqe);
		free(chunk_ctx[idx].chunk_copy);
		if (cqe_res < 0) {
			nvme_show_error("NVMe Copy CQE error: %s", strerror(-cqe_res));
			ret = cqe_res;
			break;
		}
		inflight--;
